(define (domain zenotravel)
  ...
  (:action complete-boarding
    ...
    :effect (and (increase (total-cost) 1) (probabilistic 1/2 (and (not
    (boarding ?p ?a)) (in ?p ?a) (not-boarding ?p))))
  )
  ...
  (:action complete-debarking
    ...
    :effect (and (increase (total-cost) 1) (probabilistic 1/4 (and (not (debarking ?p
    ?a)) (at-person ?p ?c) (not-debarking ?p))))
  )
  (:action start-flying
    ...
    :effect (and (increase (total-cost) 1) (not (at-aircraft ?a ?c1)) (flying ?a
    ?c2))
  )
  ...
  (:action start-zooming
    ...
    :precondition (and (at-aircraft ?a ?c1) (fuel-level ?a ?l1) (next ?l2 ?l1) (not-refueling ?a)
                       (forall (?p - person) (and (not-boarding ?p) (not-debarking ?p))))
    :effect (and (increase (total-cost) 1) (not (at-aircraft ?a ?c1)) (zooming
    ?a ?c2))
  )
  ...
  (:action start-refueling
    ...
    :effect (and (increase (total-cost) 1) (refueling ?a) (not (not-refueling
    ?a)))
  )
  (:action complete-refuling
    :parameters (?a - aircraft ?l ?l1 - flevel)
    :precondition (and (refueling ?a) (fuel-level ?a ?l) (next ?l ?l1))
    :effect (and (increase (total-cost) 1) (probabilistic 1/7 (and (not
    (refueling ?a)) (not-refueling ?a) (fuel-level ?a ?l1) (not (fuel-level ?a
    ?l)))))
  )
)
