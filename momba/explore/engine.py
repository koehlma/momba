...
class NamespaceBuilder:
    ...
    def define(self, name: str, value: expressions.Expression) -> None:
        ...
        for identifier in value.identifiers:
            if not self.has_value(identifier.name):
                pending_definition.uses.add(identifier.name)
                if identifier.name not in self.deferred:
                    self.deferred[identifier.name] = set()
                self.deferred[identifier.name].add(pending_definition)
        if not pending_definition.uses:
...
def _contains_clock_identifier(
    ...
    for identifier in expression.identifiers:
        if scope.lookup(identifier.name).typ == types.CLOCK:
            return True
    return False
        ...
        if isinstance(head, expressions.Boolean):
            ...
            if isinstance(difference, expressions.Identifier):
                ...
                assert (
                    ...
                    and difference.operator is operators.ArithmeticOperator.SUB
                    and isinstance(difference.left, expressions.Identifier)
                    and isinstance(difference.right, expressions.Identifier)
                )
                left = ClockVariable(difference.left.name, instance)
                right = ClockVariable(difference.right.name, instance)
            ...
            conjuncts.append(head)
    if conjuncts:
        result = evaluation.evaluate(expressions.land(*conjuncts), environment)
...
class MombaPTA(pta.PTA[GlobalState, Action, ClockVariable]):
    ...
    def _compute_destination(
        ...
        t.Tuple[fractions.Fraction, pta.Destination[GlobalState, ClockVariable]]
    ]:
        ...
        for instance, destination in destination_vector.destinations.items():
            for assignment in destination.assignments:
                assert isinstance(
                    assignment.target, effects.Identifier
                ...
                if typ == types.CLOCK:
                    ...
                    reset.add(ClockVariable(assignment.target.name, instance))
                else:
                    ...
                    concurrent_assignments[assignment.index].add(
                        DeferredAssignment(
                            instance, assignment.target.name, assignment.value
                        )
                    )
        ...
        for instance, state in global_state.instances.items():
            ...
            if instance_invariant is None:
                return None
            invariant |= instance_invariant
        pta_destination = pta.Destination(
            frozenset(reset), ta.Location(frozenset(invariant), global_state)
        )
    ...
    def _compute_edge(
        ...
        *,
        link: t.Optional[model.Synchronization] = None,
...
class EdgeVector:
    ...
    result_pattern: t.Optional[action.ActionPattern] = None
...
def _compute_edge_vector(
    vector_patterns: t.Mapping[automata.Instance, action.ActionPattern],
    ...
    *,
    ...
    for instance, vector_pattern in vector_patterns.items():
        ...
        for vector_argument, instance_argument in zip(
            ...
            if isinstance(vector_argument, action.GuardArgument):
                try:
                    slot = named_slots[vector_argument.name]
                except KeyError:
                    slot = VectorSlot(vector_argument.name)
                    named_slots[vector_argument.name] = slot
            else:
                slot = VectorSlot()
            vector_slots.append(slot)
            if isinstance(vector_argument, action.WriteArgument):
                slot.expressions.add(SlotExpression(vector_argument.expression))
            elif isinstance(vector_argument, action.ReadArgument):
                slot.targets.add(SlotIdentifier(vector_argument.name))
            if isinstance(instance_argument, action.WriteArgument):
                slot.expressions.add(
                    SlotExpression(instance_argument.expression, instance)
                )
            elif isinstance(instance_argument, action.ReadArgument):
                slot.targets.add(SlotIdentifier(instance_argument.name, instance))
            else:
                assert isinstance(instance_argument, action.GuardArgument)
                slot.aliases.add(SlotIdentifier(instance_argument.name, instance))
