...
In [Model Exploration](exploration), we demonstrate how Momba's explicit state space exploration engine can be used to validate a model by interactive simulation or connecting it to a testing framework such as [pytest](https://pytest.org/).
...
This user guide is meant to be read from start to finish as the sections build on top of each other.
